...
import sqlite3
import pandas as pd  # Importing pandas for better table display

class DatabaseManager:
    def __init__(self, db_file="farmers_crop_production.db"):
        self.db_file = db_file
        # Set check_same_thread=False for Streamlit's multithreading
    ...
    def add_crop(self, name):
        """Adds a new crop to the database. Added UNIQUE constraint in table."""
        try:
            cursor = self.connection.cursor()
    ...
    def get_production_by_farmer(self, farmer_id):
        """Retrieves all production records for a specific farmer."""
        cursor = self.connection.cursor()
        cursor.execute('''
    ...
    if choice == "Add Farmer":
        ...
        with st.form("add_crop_form", clear_on_submit=True):
            ...
            if submitted:
                if not crop_name:
                    ...
                    # Error/Warning is handled inside the db.add_crop method
        ...
        if st.button("Show Report"):
            ...
            if records:
                ...
                # Use Pandas DataFrame for a clean table
